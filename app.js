...
analyzeBtn.addEventListener("click", async () => {
  try {
    ...
    const officialData = normalizeRows(officialRows, "官方表", defaultCost);
    const serviceData = normalizeRows(serviceRows, "客服表", defaultCost);

    const officialFiltered = filterByStatus(officialData);
    const serviceFiltered = filterByStatus(serviceData);
...
function normalizeRows(rows, sourceName, defaultCost) {
  ...
  if (!mapping.orderNo || !mapping.status) {
    throw new Error(`${sourceName}无法识别“订单号/状态”列，请检查表头名称`);
  }
